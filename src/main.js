...
const ENEMY_FACE_TEXTURE_URL = "https://upload.wikimedia.org/wikipedia/commons/c/ca/Epstein_Final_Mugshot.png";
const HAWKING_FACE_TEXTURE_URL =
  "https://commons.wikimedia.org/wiki/Special:Redirect/file/Stephen_hawking_2008_nasa_cropped.jpg?width=512";
const TRUMP_FACE_TEXTURE_URL =
  "https://commons.wikimedia.org/wiki/Special:Redirect/file/Donald_Trump_official_portrait_%283x4a%29.jpg?width=512";
...
scene.fog = new THREE.FogExp2(0x010505, LOW_SPEC_MODE ? 0.022 : NORMAL_QUALITY_MODE ? 0.018 : 0.014);
...
renderer.toneMappingExposure = LOW_SPEC_MODE ? 0.62 : ULTRA_QUALITY_MODE ? 0.72 : 0.66;
renderer.shadowMap.enabled = ENABLE_SHADOWS;
renderer.shadowMap.type = THREE.PCFSoftShadowMap;
renderer.domElement.tabIndex = 0;
dom.shell.append(renderer.domElement);

const flashlight = new THREE.SpotLight(0xe8fbff, 68, 38, Math.PI / 11, 0.24, 1.62);
flashlight.position.set(0, 0.08, 0.08);
flashlight.target.position.set(0, -0.1, -12);
...
function addLights() {
  const hemi = new THREE.HemisphereLight(0x4f8a9c, 0x030403, LOW_SPEC_MODE ? 0.16 : 0.22);
  scene.add(hemi);

  const moon = new THREE.DirectionalLight(0x7ebbe0, LOW_SPEC_MODE ? 0.035 : 0.06);
...
function addSky() {
  ...
  const moonSphere = new THREE.Mesh(
    ...
    new THREE.MeshBasicMaterial({ color: 0xd4eaff, transparent: true, opacity: 0.34 }),
  );
...
function updateCamera(delta) {
  ...
  const activeFlashlightIntensity = hunters.some((hunter) => hunter.state.mode === "chase") ? 52 : 74;
